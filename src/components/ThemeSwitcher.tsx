...
export function ThemeSwitcher() {
  ...
  if (!mounted) {
    return (
      <div className="p-1.5 rounded-xl bg-white/60 dark:bg-slate-900 midnight:bg-black/60 backdrop-blur-md">
  ...
  return (
    <div className="relative">
      <button
        onClick={toggleOpen}
        className="flex items-center justify-center p-2 rounded-xl bg-white/60 dark:bg-slate-800/60 midnight:bg-white/10 backdrop-blur-xl border border-gray-200/50 dark:border-gray-700 midnight:border-white/10/50 midnight:border-white/10 hover:bg-gray-100 dark:hover:bg-slate-700/60 midnight:hover:bg-white/20 transition-all shadow-sm"
        title="Change theme"
      >
        {getIcon()}
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute right-0 top-12 mt-1 w-36 rounded-2xl bg-white/80 dark:bg-slate-900 midnight:bg-black/80 backdrop-blur-2xl border border-gray-200/50 dark:border-gray-700 midnight:border-white/10/50 midnight:border-white/10 shadow-xl overflow-hidden z-50 flex flex-col p-1"
          >
            <button
